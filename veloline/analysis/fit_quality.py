...
from veloline.metaparams import FIT_QUALITY_GENE_LIST, FIT_QUALITY_ADT_RNA_MAP
...
def _genes_from_adt_map(adt_rna_map):
    if not adt_rna_map:
        return []
    genes = []
    seen = set()
    for _adt, rna in adt_rna_map.items():
        if not rna:
            continue
        if rna not in seen:
            genes.append(rna)
...
def run(data_to_fit, mp, posteriors, plot_dir, n_genes=6):
    ...
    if FIT_QUALITY_GENE_LIST:
        candidate_genes = FIT_QUALITY_GENE_LIST
    else:
        candidate_genes = _genes_from_adt_map(FIT_QUALITY_ADT_RNA_MAP)
    requested_genes = _filter_gene_list(gene_names_arr, candidate_genes)
    saved = []
