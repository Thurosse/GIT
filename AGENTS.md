...
[veloline/splines_torch_fixed.py](veloline/splines_torch_fixed.py) is the device-aware torch B-spline basis module, imported by both forms.
...
[veloline/splines_torch_fixed.py](veloline/splines_torch_fixed.py) provides device-aware B-spline basis evaluation, imported by both `veloline/models.py` and the notebooks. Two recursion implementations exist: `torch_B` (faster, mask-indexed) and `tvect_B` (fully vectorial via `torch.where`, kept for reference). Prefer `torch_B` for production use.
